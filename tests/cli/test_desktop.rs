...
//!
//! Provides helpers to create dummy windows on the **current desktop**,
//! start `stmd` without desktop isolation, and query/stop it via IPC.
//!
...
impl TestWindow {
    /// Creates a visible top-level window on the current desktop.
...
/// Start `stmd` in test mode (no desktop isolation).
pub fn start_test_daemon(pipe: &str) -> Result<std::process::Child, String> {
...
pub fn query_windows(pipe: &str) -> Result<serde_json::Value, String> {
    ...
    // SAFETY: set_var is safe in single-threaded test code.
    unsafe { std::env::set_var("STM_PIPE_NAME", pipe) };
...
pub fn stop_test_daemon(pipe: &str) {
    ...
    unsafe { std::env::set_var("STM_PIPE_NAME", pipe) };
    let _ = transport::send_message(&SocketMessage::Stop);
}
