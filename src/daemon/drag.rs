...
///
/// The edge-scroll auto-repeat state lives on the orchestrator
/// (`edge_scroll` / `edge_scroll_timings` / `edge_scroll_deadline` on
/// [`FlowWM`]), NOT on this variant: it is a single shared scheduler fed by
/// both the drag feed (here, during a `Translate` drag) and the hover
/// edge-dwell feed, so it must outlive any one drag variant. Only a
/// `Translate` drag actually arms it; `Classifying` and `Resize` never scroll.
...
impl FlowWM {
    ...
    fn classify_and_promote(&mut self, hwnd: isize) {
        ...
        match classify_drag(classifying.start_rect, current_rect) {
            ...
            DragKind::Translate => {
                // The edge-scroll scheduler / timings / deadline live on the
                // orchestrator (armed fresh at `on_drag_start`), not on this
                // variant — see `DragState`.
    ...
    fn on_translate_move(&mut self, hwnd: isize) {
        ...
        if new_dir != prev_dir {
            // Leaving any armed band cancels the timer. This handler only runs
            // during a `Translate` drag (see `on_drag_move` dispatch), so the
            // shared orchestrator scheduler is the one armed here.
            if prev_dir.is_some() {
    ...
    /// scheduler is a single instance on the orchestrator (shared with the hover
    /// edge-dwell feed), so it is reset here regardless of which variant ran.
    pub(super) fn on_drag_end(&mut self, _hwnd: isize) {
        ...
        // to the float-sync path, and tear down the shared edge-scroll scheduler
    ...
    /// (a `Translate` drag) and the hover feed (`edge_dwell` EdgeLeave).
    ...
    ///
    /// Shared tail of the entry scroll ([`Self::on_translate_move`]), each
    ...
    /// drag's `Translate` move handler during a drag, the hover edge-dwell
    /// otherwise); they never overlap because the hover subsystem is suppressed
    /// while a drag is active (see [`Self::on_drag_start`]).
